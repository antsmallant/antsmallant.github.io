...
## 状态帧？
这个说法我是在 烟雨迷离半世殇 ( 知乎大佬： https://www.zhihu.com/people/gu-gao-de-wang-1 ，博客： https://www.lfzxb.top/ ) 的文章[10]里看到的。在我对网络同步有足够多的研究之前，看到 “状态帧” 这种叫法是有点懵的。在了解了更多之后，我才搞清楚 “状态帧” 其实就是守望先锋里面用到的状态同步。但实际上守望先锋自己的技术分享里面也说了，他们用的就是状态同步而已，只不过采用了多种优化手段，包括：。所以，“状态帧” 的说法，在我看来，只是增加了理解负担而已。当然，每个人都有表达和创作的自由，respect。  


## 适用范围
帧同步与状态同步的区别，我觉得可以打这么一个粗略的比方：帧同步相当于所有的客户端播放同样一部电影，视觉上是完全一样的，而状态同步是所有客户端拿着相同的剧本（状态一样），但是电影画面略有差异。而快照同步，由于所有的仿真都是在服务器之上，所以各个客户端也相当于是在播放同一部电影。  

弱客户端，像小游戏，云游戏，都可以使用快照同步。  

对于面画有精确要求的，比如格斗类游戏，玩家的出招肯定是强依赖于当前对手的出招的，所以保持双方视觉上的一致很有必要，此时就适用帧同步，除此外，还有体育类、rts 等有精细操作的。

其他类型的，基本上都可以使用状态同步。  

王者荣耀使用的是帧同步，本来这种 moba 是可以使用状态同步的，并且使用帧同步带来的麻烦很多，但在这个分享里[11]说明了选择帧同步的理由：   

...
共享内存的本质就是把不同进程的各自的一块虚拟内存空间映射到一块公共的物理内存上，这样一来，一个进程往这块内存写数据，另一个进程马上就可以读取。这是最快的一种 IPC 了。     

但这也带来一个 race condition 的问题，不同进程读写同一块内存的时候会出现冲突。要解决这种问题，还是需要锁。这时，可以使用进程间的锁：信号量。    

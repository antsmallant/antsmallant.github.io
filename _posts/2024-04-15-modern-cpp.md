...
上面例子中，要特别注意的情况是：`string&& s1 {"hello"};`，在这里，s1 是一个类型为 "string 右值引用" 的左值，当我们把 右值引用 当成一种类型之后，就比较好理解 s1 是一个左值的事实了，它是地址（内存位置）的变量。  

再举一个例子：`void f(int&& p1);`，在这个函数声明中，p1 是一个类型为 `int 右值引用` 的左值。  
...
简单的说，右值引用的目的在于提高运行效率，把复制对象变成移动对象。   

...
* 赋值表达式的结果
赋值表达式本身是有结果的，它的结果就是 = 号左侧的运算对象，比如 `a = 100`，它的结果就是 a，可以这样验证： 
...
### 左值引用
以 c++11 出现之前，引用就是指左值引用。左值引用不能绑定到临时对象上，像下面这样是非法的：  
...
### const 引用
const 引用是一种左值引用，但比较特殊，它可以绑定到右值上，像下面这些都是合法的：  
...
### 右值引用
右值引用是 c++11 引入的新概念，用两个 `&` 即 `&&` 代表。右值引用就是绑定到右值上的引用，要用 && 而不是 & 来获得右值引用。   

前置递增/递减 && 后置递增/递减，返回值是不一样，比如：  
++i 返回的是左值，过程是直接把 i 加 1，然后返回 i。
i++ 返回的是右值，过程是先用一个临时变量保存i的值，然后把 i 值加1，然后返回临时变量，之后系统会释放这个临时变量。
...
* 不要固步自封，如果使用 c++ 编程，那就要去学习和掌握 c++ 新版本的新特性，这些都可能带来生产力的提升。  

...
执行 `f(i)` 的时候，`T` 是匹配为 `int&` 的，产生的模板实例是： `f<int&>(int& && x);`。   

但这种 "reference-to-reference" （引用的引用） 有意义吗？ `int& && x` 是合法的存在吗？  

c++11 之后是合法的，不过只允许编译器内部这么产生。c++11 引入了对于 reference-to-reference 的处理，在模板实例化的时候，进行 "reference collasping"（引用折叠）。对 `int& && x` 进行处理后的结果是 `int& x`，即消除掉了 `&&`。   

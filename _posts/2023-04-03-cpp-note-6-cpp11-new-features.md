...
2、对 `std::shared_ptr` 本身的操作是线程安全的，在修改引用计数时，使用了原子操作，所以可以在多线程环境下，安全的复制和销毁指向同一个对象的 `shared_ptr` 实例。但是，对所指对象的操作不一定是线程安全的，这个仍然需要用线程同步的方法来保证线程安全。    

3、防止循环引用，如果2个或多个 `std::shared_ptr` 互相持有对方，那么可能就会阻止引用计数降为零，导致内存泄漏。     

...
原始的 gc 是标记清除式的，一个 gc 过程是原子的，中间不可被打断。   
...
此处以 lua-5.4.3 的源码为例。增量式 gc，即增量式的标记清除。   
...
几乎每个版本都对增量式 gc 算法有所改进，每次改进可能会细化 gc 步骤，lua-5.4 的 gc 步骤定义如下（取自 lgc.h ）：  
...
lua5.2 开始，就引入了分代式 gc，当时只是试验性质的，默认还是增量式。但是由于实现上不太成熟，实际运行效果不佳，在 5.3 版本被删掉了。到了 lua-5.4，它又被重新实现出来。   
...
`collectgarbage("count")` 的作用是返回 lua vm 的内存占用。几乎没有性能消耗，它只需要拿内部的几个字段就可以计算出内存消耗。对于 lua-5.4.3，计算的函数是 `gettotalbytes`，用到的字段是 `global_State` 结构体里的 `totalbytes` 和 `GCdebt` 字段。  

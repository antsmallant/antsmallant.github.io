...
* 数据库的性能上限是多少。 
* 数据库在特定硬件下的大致性能表现。 
* 自己的业务特点：是读多写少，还是读少写多，读写比例大致是多少？数据量是什么规模？
* 随着业务增长，数据库能怎么扩容？是 scale up 还是 scale out？
...
说明1：tps 通常是低于 qps 的，因为一个事务往往是包含好几条 sql 语句的。   
说明2：sql 语句不止 select/insert/delete/update 这些，像 `set autocommit = 1;` 这种也会当成一条 sql 语句统计的。   

下面都用 qps 描述 mysql / mongodb / redis 的性能，意为每秒读写次数。  
...
网上会有一些自建自测的文章，但实际上这个挺难测的，测试结果跟测试方法有很大关系，比如点数据读写，区间数据读写，差别就很大了，再有，索引读写，非索引读写，也影响很大。  

所以我直接上各大公有云参考上面的性能白皮书，虽然都有各自的立场，但数据应该不至于离谱，有参考价值。  

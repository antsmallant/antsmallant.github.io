...
matchsvr 是匹配服务器，它的最基本工作就是接受玩家的匹配请求，把这些玩家放到一个池子里，然后把匹配到一起的 n 个玩家放到某个 battlesvr 上，让它们连上去战斗。工作的时候，它可以把所有玩家的匹配请求写入内存数据库中，按一定频率从中取玩家出来做匹配。   
...
“数据仓库” 这里范指流水日志的存储及统计的基本设施，流水日志包括登录、注册、充值、游戏行为、财富变化等，依靠这些流水日志可以统计出留存，rto等数据报表，可以做用户画像，使用当下成熟的 log 采集+大数据计算方案即可。除了自研，还可以考虑接入第三方的统计，比如 “数数” 等。  

剩下的 mysql 、redis，最主要解决的是 sharding 问题。   

mysql 用于存储游戏的业务数据，也可以用其他数据库替代，比如 Posgresql，mongodb。  

redis 用于 1、存储游戏运行时数据，比如匹配战斗的数据；2、大型排行榜；3、一些展示性的数据 cache，其中 1跟2是有持久化需求的。  
...
公有云的话，选择也不多，华为云有一款叫 DDM [4]，可以尝试一下。  

**数据源方式**

自建的话，目前看，只有 TiDB 是稍微靠谱的。  

公有云的话，腾讯云的 TDSQL for Mysql [3]是可以的， p 厂做游戏还是有经验的，之前我司被独代的游戏，用的就是 p 厂改造的 mysql 引擎 tspider，支持透明的分库分表。  

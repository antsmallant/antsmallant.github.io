...
若干年前我在某公司任职时，参与研发过一款休闲类型的游戏，由 penguin 厂独代。研发的时候，经历了 p 厂好几轮的 TDR (Technical Design Review) 。虽然实际上线后最高同时在线只有 30 万左右，但这是游戏运营层面的问题，从服务器运行状况来看，完全可以稳稳支撑到百万以上的同时在线。       
...
虽然大型互联网应用的规模是远超游戏的，但游戏也有其特殊性，它对于延迟特别敏感，并且往往有一部分服务器节点必须是有状态的，所以游戏服务器架构有自己的难点，不能照搬互联网那一套。     
...
一切的难点都在于量级上，当 pcu 到达百万级的时候，水平扩容&负载均衡成为最关键的问题，整个架构都要设计成可以水平扩容的，除此之外，还要做到高可用、容灾。   
...
以上架构图里展示的，无论是基础的网络层、数据层，或是自己开发的游戏服务层，都做了水平扩容。  

下面具体讲下架构的各个组成部分，包括它们的作用，如何做到水平扩容，高可用，容灾。   
...
* 直接面向玩家
* 玩家在匹配成功后获得 battlesvr 的连接地址，连接后进行战斗
...
* 提供后台功能，本质上就是一个 webserver，提供后台管理和数据查看的功能，后台管理主要就包括活动管理、玩家封号、版本更新、玩家数据修改等等，数据查看主要就是看一些数据统计，留存之类的。 

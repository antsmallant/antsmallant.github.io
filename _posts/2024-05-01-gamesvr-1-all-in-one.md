...
若干年前我在某公司任职时，参与研发过一款休闲类型的游戏，由 penguin 厂独代。研发的时候，经历了 p 厂好几轮的 TDR (Technical Design Review) 。虽然实际上线后最高同时在线只有 30 万左右，但这是运营层面的问题，从服务器运行状况来看，完全可以稳稳支撑到百万以上的同时在线。       
...
华为云提供了一个支持分库分表的数据库中间件，叫 DDM[4]，它支持上面的 RDS for mysql。  

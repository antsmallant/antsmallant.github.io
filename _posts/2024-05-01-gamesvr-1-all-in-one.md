...
上面列出的服务器可以实现最基本的玩法，在有些情况下，可能需要额外增加一些功能服务器，但总原则是尽量不要盲目增加，很多功能，其实围绕数据库去设计就行了，像好友、公会、聊天这些，完全可以用 plazasvr 实现的。  

数据统计，一般就用大数据那一套，比如 spark，但其实现在不需要自己开发了，直接用 “数数” 之类的第三方，比自己开发更划算。  

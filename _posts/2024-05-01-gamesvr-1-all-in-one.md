...
这个游戏是若干年前，我在某家公司任职时，参与研发的休闲类型游戏，由 penguin 厂独代。研发的时候，经历了 p 厂好几轮的 TDR (Technical Design Review) 。虽然实际上线后 pcu 只有 30 万，但这是游戏运营层面的问题。从服务器运行状况来看，完全是可以稳稳支撑百万以上的同时在线。       
...
nginx 集群其实就是一个 7 层（http）负载均衡，如果用公有云的话，腾某云就用 CLB，阿某云用 SLB。   
...
mysql 代表存游戏业务数据的 db，是最主要的存储，用其他关系数据库，或 nosql 如 mongodb 都 ok，具备 acid 特性的都行，但由于游戏体量大，不可避免要分库分表，要选择能够透明 shard 的数据库。  
...
nginx 跟 lvs 都支持横向扩容，直接使用公有云上提供的 LB 组件是最方便的。比如腾某云的 CLB 就号称支持亿级连接，千万级并发。  
...
mongodb 太适合游戏使用了，它本身就支持 shard，在这种 kv 场景下，用起来没问题，像鸣潮这种大世界，用的就是 mongodb，但不清楚具体是用 mongodb 的自动 sharding，

...
这个游戏是若干年前，我在某公司任职时，参与研发的休闲类型游戏，由 penguin 厂独代。研发的时候，经历了 p 厂好几轮的 TDR (Technical Design Review) 。虽然实际上线后最高同时在线只有 30 万左右，但这是游戏运营层面的问题，从服务器运行状况来看，完全是可以稳稳支撑百万以上的同时在线。       

做过这种架构之后，再看原神这种大 dau 的开放世界游戏，或者鸣潮这种号称有 3100 万预约的，都不觉得有啥难的，反而由于开放世界的游戏偏单机向，服务器负载更小。  

而像王者荣耀这种 pcu 接近千万级的 moba 也不难，只是个 “开房间类型” 的游戏而已，里面的难点主要是网络同步这一块。       
...
包含了 nginx 集群，lvs 集群，这一层主要是对流量做负载均衡的。  

nginx 集群是一个 7 层（http）负载均衡，配合 dns 轮循。   

lvs 集群是一个 4 层（tcp）负载均衡，配合 dns 轮循。  

如果用公有云，腾某云可以用 CLB，阿某云可以用 SLB，都支持 4 层和 7 层负载均衡，并且负载上限跟稳定性都要远胜于自建的。  
...
可以被 lvs 集群随机负载到任一台 plazasvr 接入游戏；除了战斗之外的逻辑，都由 plazasvr 提供。   
...
mysql，核心 db，用于存储游戏业务数据，是最主要的存储；用其他关系数据库或 nosql 如 mongodb 都 ok，具备 acid 特性的就行；但由于游戏体量大，不可避免要分库分表，要选择能够透明 shard 的数据库。  

redis，代表内存数据库，本架构里对内存数据库的读写很重，所以 redis 要选用分布式版本的，或者公有云提供的 redis 兼容的自研内存数据库都 ok 的，关键是性能要能匹配得上业务的需要。  

etcd，这个就没有替代了，主要是作为服务器选主，以及服务发现的，按正常的奇数节点部署就 ok，没什么特别的。  

数据仓库，用于存储各种流水日志，产生运营统计报表，登录、注册、充值、游戏时长、财富变化、游戏行为等等，数据量会很庞大，存到普通的关系数据库是查不动，统计不动的，用当下流行的大数据处理就很ok了，或者直接接入第三方的比如“数数”这样的专门做数据统计的平台。   
...
4 层（tcp），可以用 dns + lvs 搭建，如果是特别夸张的并发连接数，得上 F5 之类的硬件了。  

7 层（http），可以用 dns + nginx 搭建。  

<br/>

**公有云**

用公有云是最方便的，它们提供的 LB 组件都支持 4 层跟 7 层负载，比如腾某云的 CLB，号称支持亿级连接，千万级并发；阿某云的 SLB。  

这两个云我都用过，很稳定，没出过什么故障，比自建靠谱。  
...
无状态的 http web 服务器，天然支持水平扩容，基于前置的 nginx 集群实现负载均衡就行。  
...
没什么特别情况还是上公有云吧，现在各个公有云都推出了自带 “水平分表” 的云数据库，比如腾某云的 TDSQL for mysql[3]，
...
总结一下，数据库是基础设施，分库分表放在数据库这一层就 ok 了，用成熟的方案就行，不要再用游戏服务器去做了。   
...
mysql、redis、etcd 都有常规的高可用方法，用上即可。用公用云的话就更省心。  

数据仓库要看具体的，使用 log 采集的方式，基本上都不怕数据仓库宕掉，因为本地总写有 log 文件。  
...
sdksvr，集群部署，支持横向扩展，这就是 webserver，无状态，随便宕机，没啥影响。  
...
1、能不单点就不单点，在百万级 pcu 下，单点意味着性能瓶颈，也意味着单点故障风险，可能导致整个集群完全停摆。  
...
* 游戏服务器架构设计，保持清晰简洁就好了，不要盲目追求 fashion 的设计； 
* 在设计的过程中，一定要计算好业务容量； 
* 各个层面都尽量做到能负载均衡；  
...
[3] 腾讯云. TDSQL 水平分表. https://cloud.tencent.com/document/product/557/10521.  
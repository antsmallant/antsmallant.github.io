...
### 详细说说 GCSswpfinobj 和 GCSswptobefnz 阶段

这两个阶段虽然都属于 sweep，但实际上并不会清除对象，只会把对象都重新标记为 current white，因为这两个链表上的对象都有 FINALIZEDBIT 标记，`sweeplist` 用 `isdeadm` 测试这些 obj，返回的都肯定是 false，即不是 dead mark，所以只会执行标记 current white 的逻辑，即下面代码中的 else 部分的逻辑。    
...
gc 过程中原子性（即不分步）的步骤最容易导致卡顿，从上面表格中可以看得出，标记阶段的 GCSatomic 步骤最容易卡顿，其中有些操作是无法避免的，而有些操作是我们能够努力减少其消耗的，比如减少不必要的弱表，减少不必要的 `__gc` 的对象。    

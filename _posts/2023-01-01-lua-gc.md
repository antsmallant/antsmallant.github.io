...
|阶段|步骤|是否分步|作用|
|---|---|---|---|
|标记|GCSpause|分步|对根集进行一次标记|
|标记|GCSpropagate|分步|每次从 gray 链表中取一个对象出来，首先把这个对象标记为黑色，然后 mark 所有未 mark 过的子对象|
|标记|GCSenteratomic、GCSatoimic|不分步|1、再次对根集进行标记；2、标记完 gray 链表新增的所有对象；3、标记整个 grayagain 链表的所有对象；|
|清除|GCSswpallgc|分步||
...
* 根集即 mainthread、registry表、G表 这几个对象。 
* 标记算法的逻辑：如果对象会引用其他对象，则标记为灰色，并加入 gray 链表；否则直接标记为黑色。 

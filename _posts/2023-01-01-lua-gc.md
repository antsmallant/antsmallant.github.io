...
### gc 的触发条件
todo： 详细介绍触发条件，以及 gc running 的时候，是如何一再执行的。  

lua 的 gc 不是靠时间驱动的，是靠内存分配量驱动的，当达到设定的阈值时，就触发 gc。  

lua 使用债务来描述内存占用，申请内存是负债，释放内存是还债，当债务达到某个值时，就会触发 gc。  
...
lua 同时使用了这两种做法，如果黑色对象是 table 或 userdata 类型的，则使用 barrier backward，否则使用 barrier forward。采用这种方式，应该是性能上的综合考量。  
...
|参数|作用|版本|设置方法|
|---|---|---|---|
|pause|一轮gc结束后隔多久启动下轮gc|5.1~5.4|5.1~5.3: collectgarbage("setpause", Val)  ;  5.4: collectgarbage("incremental", Val, 0, 0)|
|stepmul|gc step 工作量的倍数|5.1~5.4|5.1~5.3: collectgarbage("setstepmul", Val); 5.4: collectgarbage("incremental", 0, Val, 0) |
|stepsize||5.4|collectgarbage("incremental", 0, 0, Val)|

<br/>

这些参数的意义，官方文档解释得比较清楚，以下取自 5.4 的 lua manual ( https://lua.org/manual/5.4/manual.html#2.5.1 ):      

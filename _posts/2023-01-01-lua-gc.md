...
而 GCestimate 是一个评估值，代表 当前不是垃圾的内存总量 ( an estimate of the non-garbage memory in use )。  

实际上，当前内存申请总量 = totalbytes + GCdebt ，任何时候都要维持这一条式子成立。也就是说如果 当前内存申请总量 不变，而 GCdebt 的值改变了，那  totalbytes 的值也要相应改变，来保持等式的成立。  

虚拟机初始时： totalbytes = 当前内存申请总量，GCdebt = 0，GCestimate = 0。 
...
在增量式 gc 模式下：  
* 当 GCdebt > 0 时：触发 gc，会调用一次或多次 singlestep，每个 singlestep 都会评估一定的工作量（work），每个 singlestep 都会改变 GCdebt 的值：GCdebt = GCdebt - work ，直到把 GCdebt 的值降低到 -stepsize，或者完成了一轮 gc。 

* 完成一轮 gc 时：  
    GCestimate = 当前内存申请总量  
    GCdebt = -GCestimate * pause 参数 / 100   
    totalbytes = 当前内存申请总量 - GCdebt   

pause 参数是一个以百分比为单位的变量，用于控制两轮 gc 的间隔，默认值是 200，也就意味着本轮 gc 结束后，要等内存申请总量达到当前值的 2 倍时，才会触发那一轮 gc。   
...
### 一致性原则
在整个 gc 标记阶段，lua 必须需要保证一个原则：一致性原则，即黑色对象不得引用白色对象。假设不保证这个原则，那么由于标记阶段是增量的，那么白色的对象可能就不会被扫描到，导致被错误的清除。  
...
lua 同时使用了这两种做法，如果黑色对象是 table 或 userdata，则使用 barrier backward，否则使用 barrier forward。采用这种方式，应该是性能上的综合考量。  
...
这里有一个需要注意的点，即设置元表时，元表必须已经包含了 __gc 方法，如果是设置元表之后，再给元表加上 __gc 方法，则是无效的，因为 lua vm 是在 setmetatable 的时候做 __gc 方法的检测跟判断的。  
...
![lua-gc-wks-comment1.png](https://blog.antsmallant.top/media/blog/2023-01-01-lua-gc/lua-gc-wks-comment1.png)   
<center>图1：wks 评论1</center>

![lua-gc-wks-comment2.png](https://blog.antsmallant.top/media/blog/2023-01-01-lua-gc/lua-gc-wks-comment2.png)  
...
# 参考
[1] Roberto lerusalimschy. Lua 程序设计 (第4版). 2018.6 

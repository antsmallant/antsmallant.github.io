...
几乎每个版本都对增量式 gc 算法有所改进，每次改进可能会细化 gc 步骤，各个版本的 gc 步骤如下（具体定义在 lgc.h ）。  
...
接着以 5.4 版本为例说一下具体各个 gc 步骤做的事情，gc 总体上可以分成3个阶段，如下。  
...
1、被标为灰色或黑色的对象不再被引用了；
2、被标为黑色的对象又引用了白色对象 (other-white) ；   
...
这只是一个过渡阶段，GCSpropagate 把 gray 链表扫描完之后，需要做一次原子操作，完成标记阶段的收尾。  
...
## 调参优化
普通的应用，不需要对 gc 参数进行调优的，使用默认的 gc 参数就够了。游戏服务器对于 cpu 突刺是很敏感的，它很容易引起掉帧问题。所谓掉帧就是游戏服运行过程中每秒跑的逻辑帧数不及预期，比如本来设计是每秒 20 帧，结果由于 gc 消耗了过多的 cpu 时间，导致一秒跑不了 20 帧。    
todo   

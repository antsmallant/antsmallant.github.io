...
在整个 gc 标记阶段，lua 必须需要保持一个原则：一致性原则，即黑色对象不得引用白色对象。   

假设不保证这个原则，那么由于标记阶段是增量的，那么白色的对象可能就不会被扫描到，导致被错误的清除。  

首先说一下触发条件，lua 使用债务来描述内存占用，申请内存是负债，释放内存是还债，当债务达到某个值时，就会触发 gc。  

接着说一下各个 gc 步骤做的事情，gc 总体上可以分成4个阶段，分别是：开始、标记、清除、结束。  

* GCSenteratomic 是一个过渡阶段，本身没做什么实质的工作，顺序上是这样：
GCSpropagate -> GCSenteratomic -> GCSatoimic -> GCSswpallgc ...，它只起到一个承上启下的作用。  
* 根集： mainthread、registry表、G表。 
* 标记算法：如果对象会引用其他对象，则置为灰色，并加入 gray 链表；否则置为黑色。 
...
有趣的是，lua 同时使用了这两种做法，如果黑色对象是 table 或 userdata 类型的，则使用 barrier backward，否则使用 barrier forward。采用这种方式，应该是性能上的综合考量。  
...
# 若干问题探究
## 增量式 gc 的参数
通过函数 collectgarbage 可以对 gc 做一些控制，此函数第一个参数是个字符串，表示进行何种操作，有些操作会选用整型之类的值作为第二个参数。   

比较重要的两个参数是：setpause 和 setstepmul，分别是调整垃圾收集器的 pause 参数和 stepmul 参数。  

pause 参数影响的是 gc 的间隔，控制一轮 gc 结束之后等待多久才进行下一轮 gc。它的单位是百分比，当值为 200% 时，表示当内存使用量是上一轮 gc 结束时的值的 2 倍时启动下一轮 gc；当值为 0% 时，表示上一轮 gc 结束时立即开始下一轮的 gc。  

stepmul 参数影响的是一次 gc step 做的工作量，一轮 gc 是由多个 gc step 构成的，每次 gc step 要做的工作量，是按一定算法计算出来的，stepmul 的值越大，每次 step 做的工作量越大，也就是越积极，默认值是 200%，低于 100% 会让收集集运行得很慢。 

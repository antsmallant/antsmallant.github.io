...
|plazasvr|玩家|tcp + protobuf + 长连接|支持，用户可连接任意plazasvr接入游戏|支持，由前置的lvs集群提供负载均衡|支持，冗余部署即可|半支持。如果能做到数据立即回写，那么相当于是 “支持” 的，但仍然是有小概率丢数据的风险。【注1】|
...
【注1】plazasvr 要有两个保证：1）在有条件的情况下，比如玩法或读写qps都支持，那么尽量做到数据立即回写，或者说核心数据立即回写。2）cache 的数据只能由 plazasvr 进行写库修改。  
